...
const AddressCard = () => {
    return (
        <div className=" gap-6 justify-center p-6 -mt-32">
            {locations.map((item, key) => (
                <div key={key} className="bg-white/5  flex flex-col gap-6 text-white p-6 rounded-2x shadow-lg m-5">
                    <h2 className=" text-[30px] mb-2">{item.store}</h2>
                    <p className="text-gray-300 mb-2 BonaNova flex items-center">
                        <CiLocationOn className="mr-[20px] text-xl text-[#0427B9]" /> {item.address}
                    </p>
                    <p className="text-gray-300 mb-2 BonaNova flex items-center">
                        <AiOutlineClockCircle className="mr-[20px] text-xl text-[#0427B9]" /> {item.hours}
                    </p>
                    {/* <p className="text-gray-300 mb-4 BonaNova flex items-center">
                        🧑‍💼 Manager {item.name}
                    </p> */}
                    <p className="text-gray-300 mb-4 BonaNova flex items-center">
                        <FiPhone className="mr-[20px] text-xl text-[#0427B9]" /> {item.phone}
                    </p>
                    <a href={item.directions_link} target="_blank" rel="noopener noreferrer"
                        className="bg-gray-700 hover:bg-gray-600 w-[170px] text-white font-medium py-3 px-8  transition">
                        Get Directions
                    </a>
                </div>
            ))}
